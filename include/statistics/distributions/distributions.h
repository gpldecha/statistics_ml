...
static inline double mvnpdf(const arma::vec& x,const arma::vec& mu, const arma::mat& cov){
    return 1.0/(pow(2*M_PI,(double)cov.n_rows/2)*sqrt(arma::det(cov))) * exp(-0.5*arma::dot((x - mu),arma::inv(cov) * (x - mu)));
}

static inline void regularise_covariance(arma::mat& cov,double min_x,double min_y,double min_z){
    if(cov(0,0) < min_x)
        cov(0.0) = min_x;
    if(cov(1,1) < min_y)
        cov(1,1) = min_y;
...
static inline double random_d(double a, double b) {
    return a + (((double) rand()) / (double) RAND_MAX) * (b - a);
}

static inline unsigned int random_i(unsigned int a, unsigned int b){
