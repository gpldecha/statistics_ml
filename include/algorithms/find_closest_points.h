#ifndef FIND_CLOSEST_POINTS_H_
#define FIND_CLOSEST_POINTS_H_
...
/*
    ...
    std::pair<double, points_t> find_closest_brute(const std::vector<arma::vec>& points) {
        if (points.size() < 2) {
            return { -1, { { 0, 0 }, { 0, 0 } } };
        }
    ...
    std::pair<double, points_t> find_closest_optimized(const std::vector<point_t>& xP, const std::vector<point_t>& yP)
    {
        if (xP.size() <= 3) {
